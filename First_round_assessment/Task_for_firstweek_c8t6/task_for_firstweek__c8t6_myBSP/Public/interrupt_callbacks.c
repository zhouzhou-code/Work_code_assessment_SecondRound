...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	...
	{ 
	  ...
	  sscanf((const char*)xUSART1.ReceiveData, "%*[^=]=%f!", &data_return);//从字节流中获取数据
		
    if(xUSART1.ReceiveData[0]=='A') //设置角度
		{
		  Servo_Angle=data_return;
			Set_Servo_Angle(Servo_Angle);	
		}
 			 
	  if(xUSART1.ReceiveData[0]=='L') //LED呼吸灯
	  {
		  Max_luminance=data_return; 	 
	  }
